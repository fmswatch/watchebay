...
def ilanlari_tara():
    ...
    for url in linkler:
        # Eğer linke _rss=1 eklenmemişse otomatik ekle
        if "_rss=1" not in url:
            url = url + "&_rss=1" if "?" in url else url + "?_rss=1"
            
        print(f"Taranan Filtre: {url}")
        feed = feedparser.parse(url)
        
        if not feed.entries:
            print("Bu filtrede anlık ilan akışı yok.")
            continue

        # En güncel 3 ilanı alıyoruz
